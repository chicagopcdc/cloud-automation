...
setup_gearbox_middle() {
  ...
  if [[ ! -f "$secretsFolder/gearbox-middleware.env" || ! -f "$secretsFolder/base64Authz.txt" ]]; then
    ...
    REACT_APP_GEOAPIFY_API_KEY="$(g3k_config_lookup '.global.geoapify_api_key' 2>/dev/null || true)"
...
gen3_log_info "test with: curl https://commons-host/gearbox-middleware/_status"
