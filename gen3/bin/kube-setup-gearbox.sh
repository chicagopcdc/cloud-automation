...
S3_AWS_ACCESS_KEY_ID=$(jq -r .gearbox-aws.gearbox_bucket_aws_key_id < "$(gen3_secrets_folder)/creds.json")
S3_AWS_SECRET_ACCESS_KEY=$(jq -r .gearbox-aws.gearbox_bucket_aws_access_key < "$(gen3_secrets_folder)/creds.json")
